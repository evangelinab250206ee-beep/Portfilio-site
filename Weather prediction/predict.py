...
def predict_weather(city: str, date: str, time: str = "12:00",
    ...
    for target in schema["regression_targets"]:
        ...
        if target in ("shortwave_radiation", "wind_speed_10m"):
            pred = max(0.0, pred)
        result[target] = round(float(pred), 2)

    rain_model = _load_model(schema["classification_target"])
    rain_proba = rain_model.predict_proba(X)[0, 1]
    result["rain_probability"] = round(float(rain_proba), 3)
    result["rain_prediction"] = "Yes" if rain_proba >= rain_decision_threshold else "No"
